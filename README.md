...
- **Closed type:** `type Decision = BookTrip(TripKind) | DeclineTrip` where `TripKind` is closed
- **Sink / capability:** `grant bookTrip irreversible` → `commit { bookTrip(d) }`
- **Consequence axes:** Trust, Reversibility

## Run the demo

```sh
examples/apps/travel-agent/demo.sh
```
...
- `travel-agent_safe.eide` — the correct design.
- `travel-agent_unsafe.eide` — the tempting-but-wrong version (the negative example a model must learn to reject).
